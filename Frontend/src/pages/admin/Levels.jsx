...
// suppression d'un niveau *********************************************************:
  ...
  const columns = [
    ...
    {
        name:'Actions',
        cell: (row)=>(<button className='btn btn-outline-danger' title='retirer cet Etudiant' onClick={()=>{swal({ title: 'Vous étes sur?', text: "Vous voulez Retirer Cet Etudiant(e)", icon: 'warning', buttons: true}).then((ok) => { if (ok) { deleteLevel(row._id)}});}}><i className='bi bi-trash'></i></button>),
        center: true,
    }
  ];
