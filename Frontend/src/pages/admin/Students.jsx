...
const Students = () => {
  ...
  // initialisation de l'objet a insérer
  ...
  // car par defaut : motdepass = nom.prenom
  useEffect(() => {
    setFormData((prevData) => ({
      ...prevData,
      password: `${prevData.firstname}.${prevData.lastname}`,
    }));
  }, [formData.firstname, formData.lastname]);

  // methode utiliser pour supprimer un etudiant, utiliser par dataTable react.
  const deleteStudent = async (idStudent)=>{
    try {
        const response = await axios.delete(`http://localhost:8000/api/Users/${idStudent}`, formData);
        if(response.data.success === true){
            Swal.fire({
                title: "Succès !",
                text: "Etudiant Supprimé avec succés",
                icon: "success",
  ...
  const columns = [
    ...
    {
        name:'Actions',
        cell: (row)=>(<button className='btn btn-outline-danger' title='retirer cet Etudiant' onClick={()=>{swal({ title: 'Vous étes sur?', text: "Vous voulez Retirer Cet Etudiant(e)", icon: 'warning', buttons: true}).then((ok) => { if (ok) { deleteStudent(row._id)}});}}><i className='bi bi-trash'></i></button>)
    }
  ];

  // utiliser pour préparer l'objet a insérer ************************************************************
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
        ...
        if(response.data.success === true){
            Swal.fire({
                title: "Succès !",
                text: "Etudiant ajouté avec succés",
                icon: "success",
                confirmButtonText: "OK",
            });
        ...
        <>
          ...
          <div className="container ct-img">
            ...
            <div className="d-flex justify-content-between align-items-center mt-3">
              <h3><i className="bi bi-people-fill"></i> Les Etudiants</h3>
              <button
